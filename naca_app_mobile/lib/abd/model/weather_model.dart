...
class HourlyWeather {
  ...
  factory HourlyWeather.fromWeatherApiJson(Map<String, dynamic> json) {
    try {
      return HourlyWeather(
        time: DateTime.parse(json['time']?.toString() ?? DateTime.now().toString()),
        temperature: Weather._parseDouble(json['temp_c']),
        description: json['condition']?['text']?.toString() ?? 'Not available',
        icon: json['condition']?['icon']?.toString() ?? '',
        humidity: Weather._parseInt(json['humidity']),
        windSpeed: Weather._parseDouble(json['wind_kph']) / 3.6,
        chanceOfRain: Weather._parseDouble(json['chance_of_rain']),
      );
    } catch (e) {
      debugPrint('Error parsing hourly weather data: $e');
      debugPrint('JSON data: $json');
      rethrow;
    }
  }
...
class DailyWeather {
  ...
  factory DailyWeather.fromWeatherApiJson(Map<String, dynamic> json) {
    try {
      ...
      return DailyWeather(
        date: DateTime.parse(json['date']?.toString() ?? DateTime.now().toString()),
        maxTemperature: Weather._parseDouble(dayData?['maxtemp_c']),
        minTemperature: Weather._parseDouble(dayData?['mintemp_c']),
        description: dayData?['condition']?['text']?.toString() ?? 'Not available',
        icon: dayData?['condition']?['icon']?.toString() ?? '',
        humidity: Weather._parseInt(dayData?['avghumidity']),
        windSpeed: Weather._parseDouble(dayData?['maxwind_kph']) / 3.6,
        chanceOfRain: Weather._parseDouble(dayData?['daily_chance_of_rain']),
        totalPrecipitation: Weather._parseDouble(dayData?['totalprecip_mm']),
      );
    } catch (e) {
      debugPrint('Error parsing daily weather data: $e');
      debugPrint('JSON data: $json');
      rethrow;
    }
  }

  String get maxTemperatureString => '${maxTemperature.round()}°C';
  String get minTemperatureString => '${minTemperature.round()}°C';
  String get temperatureRangeString => '${maxTemperature.round()}°/${minTemperature.round()}°';
  String get dayName {
  ...
  String get humidityString => '$humidity%';
  String get chanceOfRainString => '${chanceOfRain.round()}%';
  String get precipitationString => '${totalPrecipitation.toStringAsFixed(1)}mm';
  String get iconUrl => 'https:$icon';
